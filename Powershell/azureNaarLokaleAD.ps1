...
if (-not $ouParentExists) {
    New-ADOrganizationalUnit -Name $ouParentName -ProtectedFromAccidentalDeletion $false
    Write-Host "OU '$ouParentName' succesvol aangemaakt."

    # Maak de sub-OU aan
    New-ADOrganizationalUnit -Name 's122861' -Path "OU=$ouParentName,DC=sebas-tien.com,DC=local"
    ...
    New-ADOrganizationalUnit -Name $ouGroupsName -Path "OU=$ouChildName,OU=$ouParentName,DC=sebas-tien.com,DC=local" -ProtectedFromAccidentalDeletion $false
    Write-Host "OU '$ouGroupsName' succesvol aangemaakt binnen OU '$ouChildName'."
    New-ADOrganizationalUnit -Name $ouUsersName -Path "OU=$ouChildName,OU=$ouParentName,DC=sebas-tien.com,DC=local" -ProtectedFromAccidentalDeletion $false
